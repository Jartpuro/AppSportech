...
@Component({
  selector: 'sportechapp-training-program',
...
export class TrainingProgramComponent implements OnInit {
  ...
  loadPage(page?: number, dontNavigate?: boolean): void {
    ...
    this.trainingProgramService
      ...
      .subscribe({
        next: (res: HttpResponse<ITrainingProgram[]>) => {
  ...
  trackId(_index: number, item: ITrainingProgram): number {
    return item.id!;
  }

  delete(trainingProgram: ITrainingProgram): void {
    const modalRef = this.modalService.open(TrainingProgramDeleteDialogComponent, { size: 'lg', backdrop: 'static' });
    modalRef.componentInstance.trainingProgram = trainingProgram;
  ...
  protected onSuccess(data: ITrainingProgram[] | null, headers: HttpHeaders, page: number, navigate: boolean): void {
