...
@Component({
  template: `
    <table>
      <thead>
        <tr sportechappSort [(predicate)]="predicate" [(ascending)]="ascending" (sortChange)="transition($event)">
          <th sportechappSortBy="name">ID<fa-icon *ngIf="sortAllowed" [icon]="'sort'"></fa-icon></th>
        </tr>
      </thead>
    </table>
  `,
...
describe('Directive: SortByDirective', () => {
  ...
  it('should initialize predicate, order, icon when initial component predicate differs from column predicate', () => {
    ...
    // THEN
    expect(sortByDirective.sportechappSortBy).toEqual('name');
